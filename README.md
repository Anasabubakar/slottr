> [!WARNING]  
> Use at your own risk. This is an independent, self-hosted scheduling platform, originally forked from Cal.com's open-source codebase. It is strictly recommended for personal, non-production use until you've reviewed and replaced the TODO placeholders throughout this repo (branding, legal pages, support contacts, etc). Self-hosting requires advanced knowledge of server administration, database management, and securing sensitive data. Proceed only if you are comfortable with these responsibilities.

<!-- PROJECT LOGO -->
<p align="center">
  <h3 align="center">Slottr</h3>
...
**Slottr** is an independent, fully open-source scheduling platform, originally forked from Cal.com's open-source (non-enterprise) codebase.

Slottr is **100% MIT-licensed** with no proprietary "Enterprise Edition" features. It's designed for individuals and self-hosters who want full control over their scheduling infrastructure without any commercial dependencies.

### About this fork

- **No enterprise features** — Teams, Organizations, Insights, Workflows, SSO/SAML, and other EE-only features were removed by the upstream fork this project started from
- **No license key required** — Everything works out of the box, no external account or license needed
- **100% open source** — The entire codebase is licensed under MIT, no "Open Core" split
- **Independent project** — This is not affiliated with, endorsed by, or supported by Cal.com, Inc. Contributions go directly into this project (see [CONTRIBUTING.md](./CONTRIBUTING.md))

> **Note:** Slottr is a self-hosted project. There is no hosted/managed version. You run it on your own infrastructure.
...
Here’s what you need to run Slottr.
...
TODO: publish your own Docker image and update this link (was previously DockerHub at calcom/cal.diy). Until then, build the image locally with the `Dockerfile` in this repo — see the Docker Compose steps below.
...
#### Running Slottr with Docker Compose
...
5. Start Slottr via docker compose

   To run the complete stack, which includes a local Postgres database, Slottr web app, and Prisma Studio:

   ```bash
   docker compose up -d
   ```

   To run Slottr web app and Prisma Studio against a remote database, ensure that DATABASE_URL is configured for an available database and run:

   ```bash
   docker compose up -d calcom studio
   ```

   To run only the Slottr web app, ensure that DATABASE_URL is configured for an available database and run:
...
6. Open a browser to [http://localhost:3000](http://localhost:3000), or your defined NEXT_PUBLIC_WEBAPP_URL. The first time you run Slottr, a setup wizard will initialize. Define your first user, and you're ready to go!
...
4. Build the Slottr docker image:
...
5. Build Slottr via docker compose (DOCKER_BUILDKIT=0 must be provided to allow a network bridge to be used at build time. This requirement will be removed in the future)

   ```bash
   DOCKER_BUILDKIT=0 docker compose build calcom
   ```

6. Start Slottr via docker compose
...
7. Open a browser to [http://localhost:3000](http://localhost:3000), or your defined NEXT_PUBLIC_WEBAPP_URL. The first time you run Slottr, a setup wizard will initialize. Define your first user, and you're ready to go!
...
| --- | --- | --- | --- |
| DATABASE_URL | database url with credentials - if using a connection pooler, this setting should point there | required | `postgresql://unicorn_user:magical_password@database:5432/app_db` |
...
| NEXTAUTH_URL | Location of the auth server. By default, this is the Slottr docker instance itself. | optional | `{NEXT_PUBLIC_WEBAPP_URL}/api/auth` |
...
| CALCOM_TELEMETRY_DISABLED | Allow Slottr to collect anonymous usage data (set to `1` to disable) | optional | |
...
Slottr is fully open source, licensed under the [MIT License](https://opensource.org/license/mit).

Unlike Cal.com's "Open Core" model, Slottr has **no commercial/enterprise code**. The entire codebase is available under the same open-source license.
...
9. Under Authorized redirect URI's, select Add URI and then add the URI `<Slottr URL>/api/integrations/googlecalendar/callback` and `<Slottr URL>/api/auth/callback/google` replacing Slottr URL with the URI at which your application runs.
10. The key will be created and you will be redirected back to the Credentials page. Select the newly generated client ID under OAuth 2.0 Client IDs.
11. Select Download JSON. Copy the contents of this file and paste the entire JSON string in the `.env` file as the value for `GOOGLE_API_CREDENTIALS` key.

#### _Adding google calendar to Slottr App Store_
...
1. Add extra redirect URL `<Slottr URL>/api/auth/callback/google`
...
4. Set the **Web** redirect URI to `<Slottr URL>/api/integrations/office365calendar/callback` replacing Slottr URL with the URI at which your application runs.
...
8. Set the "OAuth Redirect URL" under "OAuth Information" as `<Slottr URL>/api/integrations/zoomvideo/callback` replacing Slottr URL with the URI at which your application runs.
...
11. Click "Done".
12. You're good to go. Now you can easily add your Zoom integration in the Slottr settings.
...
5. Set the Redirect URL for OAuth `<Slottr URL>/api/integrations/basecamp3/callback` replacing Slottr URL with the URI at which your application runs.
...
7. Set the Redirect URL for OAuth `<Slottr URL>/api/integrations/hubspot/callback` replacing Slottr URL with the URI at which your application runs.
...
10. You're good to go. Now you can see any booking in Slottr created as a meeting in HubSpot for your contacts.
...
7. Set the Redirect URL for OAuth `<Slottr URL>/api/integrations/zohocrm/callback` replacing Slottr URL with the URI at which your application runs.
...
10. You're good to go. Now you can easily add your ZohoCRM integration in the Slottr settings.
...
Slottr uses [Unkey](https://unkey.com) for rate limiting. This is an optional feature and is not required for self-hosting.
...
Note: If you don't configure Unkey, Slottr will work normally without rate limiting enabled.

## Contributing

We welcome contributions! Whether it's fixing a typo, improving documentation, or building new features, your help makes Slottr better.

> **Important:** Slottr is a community fork. Contributions to this repo do **not** flow to Cal.com's production platform. See [CONTRIBUTING.md](./CONTRIBUTING.md) for details.
...
Don't code but still want to contribute? help translate Slottr into your language.

<!-- ACKNOWLEDGEMENTS -->

## Acknowledgements

Slottr is built on the foundation created by [Cal.com](https://cal.com) and the many contributors to the original project. Special thanks to:
