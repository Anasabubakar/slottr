...
import { getUsernameFromCalLink, subscribeToRealtimeSlotChanges } from "./lib/realtimeSync";
import { resolveApiKeyToCalLink } from "./lib/resolveApiKey";
import {
...
export class Cal {
  ...
  /**
   * Closes the current realtime WebSocket subscription (if any), used when the iframe
   * loads a new calLink and we need to subscribe to a different owner's channel.
   */
  private unsubscribeFromRealtimeSlotChanges: (() => void) | null = null;
  ...
  createIframe({
    ...
    this.subscribeToRealtimeSlotChangesForCalLink(calLink);
    return iframe;
  }

  /**
   * Subscribes to the realtime service for the booking-page owner's channel so we can tell
   * the iframe to refetch slots when their calendar changes. No-op if the realtime service
   * isn't configured or the calLink's owner can't be determined - this is a best-effort
   * enhancement on top of the normal load-time/polling slot fetch.
   */
  private subscribeToRealtimeSlotChangesForCalLink(calLink: string) {
    this.unsubscribeFromRealtimeSlotChanges?.();
    this.unsubscribeFromRealtimeSlotChanges = null;

    const channel = getUsernameFromCalLink(calLink);
    if (!channel) return;

    this.unsubscribeFromRealtimeSlotChanges = subscribeToRealtimeSlotChanges({
      channel,
      onSlotsChanged: () => {
        // Reuses the same "force slots re-fetch" mechanism as the connect() flow,
        // without a full iframe reload.
        this.doInIframe({
          method: "connect",
          arg: { config: this.embedConfig ?? {}, params: {} },
        });
      },
    });
  }
...
class CalApi {
  ...
  inline({
    ...
    if (!calLink && apiKey) {
      ...
      resolveApiKeyToCalLink({ apiKey, calOrigin: calConfig.calOrigin }).then((resolvedCalLink) => {
        this.__inline({ calLink: resolvedCalLink, elementOrSelector, config });
      });
      return;
    }

    this.__inline({ calLink: calLink ?? "", elementOrSelector, config });
  }
  ...
  floatingButton({
    ...
    if (!calLink && apiKey) {
      const calConfig = this.cal.getCalConfig();
      resolveApiKeyToCalLink({ apiKey, calOrigin: calOrigin || calConfig.calOrigin }).then(
        (resolvedCalLink) => {
          this.__floatingButton({
  ...
  async modal({
    ...
    if (!calLink && apiKey) {
      calLink = await resolveApiKeyToCalLink({ apiKey, calOrigin });
    }
    if (!calLink) {
      throw new Error("Either calLink or apiKey must be provided");
    }
