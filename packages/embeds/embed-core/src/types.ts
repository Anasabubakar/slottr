...
/**
 * Lets an embedder identify which user/event-type to book using only an API key generated from
 * Developer > API Keys, instead of having to know and expose the user's `calLink`
 * (`username/event-type-slug`). Provide exactly one of `calLink` or `apiKey`.
 */
export type CalLinkOrApiKey = { calLink: string; apiKey?: never } | { calLink?: never; apiKey: string };
