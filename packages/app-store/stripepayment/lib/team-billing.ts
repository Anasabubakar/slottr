...
export function getRequestedSlugError(error: unknown, requestedSlug: string) {
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
    statusCode = 400;
    message = `It seems like the requestedSlug: '${requestedSlug}' is already taken. Please contact support at support@TODO-your-domain.example so we can resolve this issue.`;
