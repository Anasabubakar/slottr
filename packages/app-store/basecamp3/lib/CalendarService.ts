...
class BasecampCalendarService implements Calendar {
  ...
  private async getBasecampDescription(event: CalendarEvent): Promise<string> {
    ...
    const baseString = `<div>Event title: ${event.title}<br/>Date and time: ${date}, ${startTime} - ${endTime} ${timeZone}<br/>View on Slottr: <a target="_blank" rel="noreferrer" class="autolinked" data-behavior="truncate" href="https://TODO-your-domain.example/booking/${event.uid}">https://TODO-your-domain.example/booking/${event.uid}</a> `;
