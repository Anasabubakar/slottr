...
import { notifyRealtimeSync } from "@calcom/lib/server/notifyRealtimeSync";
...
export class CalendarSubscriptionService {
  ...
  async processEvents(selectedCalendar: SelectedCalendar): Promise<{
    ...
    if (result.eventsCached > 0 || result.eventsSynced > 0) {
      await this.notifyRealtimeSubscribers(selectedCalendar.userId);
    }
  ...
  /**
   * Notify the realtime sync service so embed widgets subscribed to this user's
   * channel can refetch availability. Booking pages are keyed by username, so a
   * user without one (not fully onboarded) has no channel to notify.
   */
  private async notifyRealtimeSubscribers(userId: number): Promise<void> {
    const username = await this.deps.selectedCalendarRepository.findUsernameByUserId(userId);
    if (!username) return;
    notifyRealtimeSync(username);
  }
