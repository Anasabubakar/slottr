...
This file contains domain knowledge about the Slottr product and codebase. For coding guidelines and rules, see [`rules/`](rules/).
...
### Slottr Event Identification

Slottr events in Google Calendar can be identified by checking if the iCalUID ends with `@Slottr` (e.g., `2GBXSdEixretciJfKVmYN8@Slottr`). This identifier is used to distinguish Slottr bookings from other calendar events for data storage and privacy purposes.
...
Workflows and webhooks are two completely separate features in Slottr with different implementations and file structures:
