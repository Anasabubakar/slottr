---
name: calcom-api
description: Interact with the Slottr API v2 to manage scheduling, bookings, event types, availability, and calendars. Use this skill when building integrations that need to create or manage bookings, check availability, configure event types, or sync calendars with Slottr's scheduling infrastructure.
env:
  CAL_API_KEY:
    description: "Slottr API key (prefixed with cal_live_ or cal_test_). Required for all API requests."
    required: true
  CAL_CLIENT_ID:
...
This skill provides guidance for AI agents to interact with the Slottr API v2, enabling scheduling automation, booking management, and calendar integrations.
...
|---------------------|----------|-------------|
| `CAL_API_KEY` | Yes | Slottr API key (prefixed with `cal_live_` or `cal_test_`). Used as Bearer token for all API requests. Generate from Settings > Developer > API Keys. |
