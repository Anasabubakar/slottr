...
This guide covers the most common issues encountered when self-hosting Slottr, along with their solutions.
...
Related issue: [#25993](https://github.com/calcom/cal.diy/issues/25993)
...
**Symptom:** After deploying Slottr to a server or domain, login redirects or internal links point back to `http://localhost:3000` instead of your actual domain.

**Cause:** The environment variables `NEXTAUTH_URL` and `NEXT_PUBLIC_WEBAPP_URL` are not set to your production domain. These variables tell Slottr and NextAuth where the app is hosted.
...
Related issue: [#21921](https://github.com/calcom/cal.diy/issues/21921)
...
**Symptom:** Requests fail with SSL certificate errors when Slottr is behind a load balancer or reverse proxy that handles HTTPS termination.
...
1. Search the [Slottr GitHub Issues](TODO_REPOSITORY_URL/issues) — many common problems have documented solutions in issue threads.
2. Review the [Docker configuration](./docker) and [Installation guide](./installation) for any steps you may have missed.
