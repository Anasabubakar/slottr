...
TODO: publish your own Docker image and update this section with its registry link. Until then, build the image locally with the `Dockerfile` in this repo.

### Contributing

The Docker configuration for Slottr is maintained in this repository. The Dockerfile and docker-compose.yml are located in the repo root. If you want to contribute to the Docker setup, please open an issue or pull request here.
...
Start Slottr via docker compose (Most basic users, and for First Run) To run the complete stack, which includes a local Postgres database, Slottr web app, and Prisma Studio:
...
2. Open a browser to http://localhost:3000, or your defined NEXT_PUBLIC_WEBAPP_URL. The first time you run Slottr, a setup wizard will initialize. Define your first user, and you're ready to go!

### Update Calcom Instance

Stop the Slottr stack
...
For more detailed instructions on how to build and configure your own Docker image, refer to the Dockerfile and docker-compose.yml in the root of the Slottr repository.
