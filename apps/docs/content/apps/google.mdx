...
```
<Slottr URL>/api/integrations/googlecalendar/callback
<Slottr URL>/api/auth/callback/google
```

Replace `<Slottr URL>` with the URL where your application runs.
...
### Adding Google Calendar to Slottr App Store
