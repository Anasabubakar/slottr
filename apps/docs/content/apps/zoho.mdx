...
```
<Slottr URL>/api/integrations/zohocrm/callback
```

Replace `<Slottr URL>` with your application URL.
...
10. **Integration is Ready** - Your ZohoCRM integration can now be easily added in the Slottr settings.
...
```
<Slottr URL>/api/integrations/zohocalendar/callback
```
...
5. **Save Credentials in Slottr Admin Panel** - Copy the Client ID and Client Secret to your app keys in the Slottr admin panel at:

```
<Slottr>/settings/admin/apps
```
...
```
<Slottr URL>/api/integrations/zoho-bigin/callback
```
...
7. **Complete Zoho Bigin Integration** - Your Zoho Bigin integration is now ready and can be added from the Slottr app store.
