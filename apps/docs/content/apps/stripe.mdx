...
4. **Add the redirect URL** - Add the following redirect URL, replacing `<Slottr URL>` with your application's URL:

```
<Slottr URL>/api/integrations/stripepayment/callback
```
...
```
<Slottr URL>/api/integrations/stripepayment/webhook
```
